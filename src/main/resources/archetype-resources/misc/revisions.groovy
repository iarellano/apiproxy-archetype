...
if (request.getStatusCode() == 200) { // If api exists
    ...
    if (versionsCount > 0) { // If api has revisions
        ...
        properties.setProperty("apiproxy.revision.deploying", String.valueOf(Integer.parseInt(greatestRevision) + 1))
    } else { // api has no revisions
        properties.setProperty("apiproxy.revision.deploying", "1")
    }
} else if (request.getStatusCode() == 404) { // api does not exist
    properties.setProperty("apiproxy.revision.deploying", "1")
}

properties.setProperty("apiproxy.revision", properties.getProperty("apiproxy.revision.deploying"))
project.getProperties().putAll(properties)
PrintWriter pw = new PrintWriter(new File(project.getBasedir(), "revisions.properties"))
properties.list(pw)
pw.close()

log.info("apiproxy.revision.undeploying: " + properties.getProperty("apiproxy.revision.undeploying"))
log.info("apiproxy.revision.deploying: " + properties.getProperty("apiproxy.revision.deploying"))
log.info("apiproxy.revision: " + properties.getProperty("apiproxy.revision"))
